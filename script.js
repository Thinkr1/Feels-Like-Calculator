...
const calc = (temp, humid /*, press*/, wind) => {
  ...
  const waterVapourPressure =
    (humid / 100) * 6.105 * Math.exp((17.27 * temp) / (237.7 + temp)); // I could have added a field asking for the water vapour pressure directly but I prefer to proceed like that
  const ATC = temp + 0.33 * waterVapourPressure - 0.7 * kmhwind - 4.0; // Calculation in degrees Celcius
...
button.addEventListener("click", (e) => {
  ...
  if (
    ...
    var result = calc(
      ...
      parseFloat(humid.value) /*, parseFloat(press.value)*/,
